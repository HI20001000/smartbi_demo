...
補充：若要導入 LLM 輔助補全，可參考 `docs/llm_json_completion_implementation_plan.md`。


## 7. LLM 輔助補全 JSON 建議放在哪一段？

建議預設放在 **`validate_normalized_request(...)` 之前**，也就是：

1. `build_normalized_request(...)` 先產生初稿 JSON
2. LLM 做「受控補全」（只補缺漏、不要覆寫高信心欄位）
3. `validate_normalized_request(...)` 做最終硬性驗證

原因：

- 驗證器是最後防線，LLM 產生的內容仍可能格式錯誤或越權。
- 先補全再驗證，能把 LLM 視為「草稿修補器」，把 deterministic validator 保留為 gatekeeper。
- 如果先驗證再補全，補完後仍需再驗一次，否則會失去資料品質保證。

實務上可採「雙階段」：

- 第一次驗證失敗時，將 `errors` 與原 JSON 丟給 LLM 進行定向修補。
- LLM 回傳後 **再次驗證**；若仍失敗則拒絕或走人工/規則 fallback。

> 結論：
> - 主路徑：LLM 補全在 validate 前。
> - 可靠性路徑：validate 失敗後可再觸發一次 LLM 修補，但一定要 re-validate。
